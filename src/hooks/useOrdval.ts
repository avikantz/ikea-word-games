...
import { OrdvalRound, IKEAProduct, GAME_MODE } from "@/interfaces";

interface UseOrdvalProps {
  length?: number;
  mode?: GAME_MODE;
}

type OrdvalRoundFunction = () => OrdvalRound | undefined;

export const useOrdval = ({ length, mode }: UseOrdvalProps) => {
  ...
  const getOrdvalRound: OrdvalRoundFunction = useCallback(() => {
    if (!list || !ikeaMap) {
      return;
    }
